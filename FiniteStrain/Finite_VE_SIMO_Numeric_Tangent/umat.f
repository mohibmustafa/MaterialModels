...
C      NOTE : This version uses Numerical tangent (just for kicks :D)
C             the master branch contains the implementation with a 
C             analytically derived consistent tangent. 
C             If I were you, I would use that version!

C     Mohib Mustafa - IMDEA 11 AUG 2021
...
C     !--------------------------------------------------------------
     ...
     1   PROPS,NPROPS,DTIME,DSTRAN,KINC,KSTEP,NOEL,DFGRD0,DFGRD1)
        ...
        INTEGER    :: index, ii, jj, mm, ll, K1, K2, nn
        ...
        REAL(prec) :: delF(3, 3, 3, 3), F_hat(6, 3, 3), J_hat(6)
        REAL(prec) :: F_hat_bar(6, 3, 3), F_hat_bar_inv(6, 3, 3)
        REAL(prec) :: B_hat_bar(6, 3, 3), dev_B_hat_bar(6, 3, 3)
        REAL(prec) :: trB_hat_bar(6), tau_o_hat_bar(6, 3, 3)
        REAL(prec) :: tau_o_hat_bar_v(6, 6), S_hat_til_o(6, 3, 3)
        REAL(prec) :: H_hat(6, 3, 3), D_hat(6, 3, 3)
        REAL(prec) :: hh_hat_bar_n(6, 3, 3), hh_hat_bar_n_v(6, 6)
        REAL(prec) :: h_hat_bar_n(6), tau_hat_v(6, 6), p_hat(6)
        REAL(prec) :: tau_hat(6, 3, 3), DD1(3, 3, 3, 3)
        REAL(prec) :: DD2(3, 3, 3, 3), F_hat_mat(3, 3, 3, 3)
    ...
    !     DO ii = 1, 3
    !       DO jj = 1, 3
    !       F_hat_mat(ii, jj, :, :) = DFGRD1(:, :)+ delF(ii, jj, :, :)
    !       END DO
    !     END DO
    ...
    !     WRITE(*,*) '--------------------------------------------'
        ...
        DO K1 = 1, 6
          F_hat_bar(K1, :, :) = (J_hat(K1) ** (-ONE / THREE)) 
     1                        * F_hat(K1, :, :)

          CALL matInv(F_hat_bar(K1, :, :), F_hat_bar_inv(K1, :, :))
        END DO
    ...
    !     END DO
     ...
     1                             + F_bar(ii, K1) * F_bar(jj, K1)
        ...
        DO K2 = 1, 6
          DO K1 = 1, 3
            DO ii = 1, 3
              DO jj = 1, 3
                B_hat_bar(K2, ii, jj) = B_hat_bar(K2, ii, jj) 
     1           + F_hat_bar(K2, ii, K1) * F_hat_bar(K2, jj, K1)
              END DO
            END DO
          END DO
        END DO
        ...
        DO K1 = 1, 6
          CALL dev(B_hat_bar(K1, :, :), dev_B_hat_bar(K1, :, :))
        END DO

        trB_bar = B_bar(1, 1) + B_bar(2, 2) + B_bar(3, 3)
        DO K1 = 1, 6
          trB_hat_bar(K1) = B_hat_bar(K1, 1, 1) + B_hat_bar(K1, 2, 2)
     1                    + B_hat_bar(K1, 3, 3)
        END DO
        ...
        DO K1 = 1, 6
          tau_o_hat_bar(K1, :, :) = mu0 * dev_B_hat_bar(K1, :, :)
        END DO
        ...
        DO K2 = 1, 6
          DO K1 = 1, 3
            tau_o_hat_bar_v(K2, K1) = tau_o_hat_bar(K2, K1, K1)
          END DO
          tau_o_hat_bar_v(K2, 4) = tau_o_hat_bar(K2, 1, 2)
          tau_o_hat_bar_v(K2, 5) = tau_o_hat_bar(K2, 1, 3)
          tau_o_hat_bar_v(K2, 6) = tau_o_hat_bar(K2, 2, 3)
        END DO
        ...
        DO ll = 1, 6
          DO K1 = 1, 3
            DO K2 = 1, 3
              DO ii = 1, 3
                DO jj = 1, 3
                  S_hat_til_o(ll, ii, jj) = S_hat_til_o(ll, ii, jj) 
     1             + F_hat_bar_inv(ll, ii, K1) 
     2             * tau_o_hat_bar(ll, K1, K2) 
     3             * F_hat_bar_inv(ll, jj, K2)
                END DO
              END DO
            END DO
          END DO
     ...
     1          + EXP(-DTIME/(TWO * tau1)) * S_til_o(:, :)
        ...
        DO K1 = 1, 6
          H_hat(K1, :, :) = H_til_n(:, :) 
     1             + EXP(-DTIME/(TWO * tau1)) * S_hat_til_o(K1, :, :)
        END DO
        ...
        DO K1 = 1, 6
          p_hat(K1) = 0.5D0 * kappa * (J_hat(K1) - (ONE / J_hat(K1)))
        END DO
        ...
        DO ll = 1, 6
          DO K1 = 1, 3
            DO K2 = 1, 3
              DO ii = 1, 3
                DO jj = 1, 3
                  D_hat(ll, ii, jj) = D_hat(ll, ii, jj) 
     1                    + F_hat_bar(ll, ii, K1) * H_til_n(K1, K2) 
     2                    * F_hat_bar(ll, jj, K2)
                END DO
              END DO
            END DO
          END DO
        ...
        DO K1 = 1, 6
          CALL dev(g1 * D_hat(K1, :, :), hh_hat_bar_n(K1, :, :))
        END DO
        ...
        DO K2 = 1, 6
          DO K1 = 1, 3
            hh_hat_bar_n_v(K2, K1) = hh_hat_bar_n(K2, K1, K1)
          END DO
          hh_hat_bar_n_v(K2, 4) = hh_hat_bar_n(K2, 1, 2)
          hh_hat_bar_n_v(K2, 5) = hh_hat_bar_n(K2, 1, 3)
          hh_hat_bar_n_v(K2, 6) = hh_hat_bar_n(K2, 2, 3)
        END DO
        

        h_bar_n = g1 * (D(1, 1) + D(2, 2) + D(3, 3))
        DO K1 = 1, 6
          h_hat_bar_n(K1) =  g1 * (D_hat(K1, 1, 1) 
     1                    + D_hat(K1, 2, 2) + D_hat(K1, 3, 3))
        END DO
     ...
     1            + g_star * tau_o_bar(:, :) + hh_bar_n(:, :)
        ...
        tau_hat(:, :, :) = ZERO
        DO K1 = 1, 6
          tau_hat(K1, :, :) = J_hat(K1) * p_hat(K1) * I_mat(:, :) 
     1            + g_star * tau_o_hat_bar(K1, :, :) 
     2            + hh_hat_bar_n(K1, :, :)
        END DO
        ...
        DO K2 = 1, 6
          DO K1 = 1, 3
            tau_hat_v(K2, K1) = tau_hat(K2, K1, K1)
          END DO
          tau_hat_v(K2, 4) = tau_hat(K2, 1, 2)
          tau_hat_v(K2, 5) = tau_hat(K2, 1, 3)
          tau_hat_v(K2, 6) = tau_hat(K2, 2, 3)
        END DO
