...
C     List of internal variables:
      DIMENSION xioi(6, 6), xii(6, 6), xpp(6, 6), dev(6), So(6),
     1  So_n(6), dev_n(6)
      DOUBLE PRECISION treps, ginf, g1, trep, tau1
...
c     Compute deviatoric part of strain tensor
      ...
      DO K1 = 1, 3
            dev_n(K1)= STRAN(K1) - trep / THREE
            dev_n(K1 + 3) = STRAN(K1 + 3) / TWO
      END DO


C     So
      DO K1 = 1, 6
            So(K1) = TWO * P_MU0 * dev(K1)
      END DO
      
      DO K1 = 1, 6
            So_n(K1) = TWO * P_MU0 * dev_n(K1)
      END DO


c     Update history variables
      DO K1 = 1,6
        STATEV(K1) = EXP(-DTIME/tau1) * STATEV(K1) 
     1      + EXP(-DTIME / (TWO * tau1)) * (So(K1) - So_n(K1))
      END DO

c     Compute Stress Tensor
      DO K1=1, 3
        STRESS(K1) = P_KA * treps + ginf * So(K1) + g1 * (STATEV(K1))
            
        STRESS(K1 + 3) = ginf * So(K1 + 3) + g1 * (STATEV(K1 + 3)) 
      END DO
...
c     Compute Tangent Modulus
     ...
     1     + (ginf + g1 * EXP(-DTIME/(TWO * tau1))) 
     2     * TWO * P_MU0 * xpp(K1, K2) 
        END DO
      END DO
