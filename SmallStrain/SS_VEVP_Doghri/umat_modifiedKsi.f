C Mohib Mustafa - IMDEA 2 Feb 2021      
...
C        IF (STATEV(19) .LE. TOLER) THEN
C          res_i = mod_beta_vp_tr 
C     1          - (P_Y0 + zeta_trial * STATEV(19) / TOLER ) 
C     2          * (TWO/THREE)**0.5D0

C          G_i = fac7 + ((TWO * P_H) / (THREE * TOLER)) 
C     1        * STATEV(19)**(P_R)
C        ELSE
          res_i = mod_beta_vp_tr 
     1          - (P_Y0 + zeta_trial) * (TWO/THREE)**0.5D0

          G_i = fac7 + ((TWO * P_H * P_R) / THREE ) 
     1        * STATEV(19)**(P_R - ONE)
...
C        END IF
        ...
        DO WHILE(res_i .GT. TOLER)
          ...
          xi_i = STATEV(19) + GAM_i * (TWO / THREE)**0.5D0

C          IF (xi_i .LT. TOLER) THEN

C            G_i = fac7 + ((TWO * P_H) / (THREE * TOLER)) * xi_i**P_R 
C     1          + (P_ETA0 / (P_M * DTIME)) * ((P_ETA0 * GAM_i) 
C     2          / (P_Y0 * DTIME))**((ONE - P_M)/P_M)

C            res_i = mod_beta_vp_tr - fac7 * GAM_i 
C     1       - (P_Y0 + h * (xi_i / TOLER) * xi_i**P_R) 
C     2       * (TWO/THREE)**0.5D0 
C     3       - P_Y0 * ((GAM_i * P_ETA0) / (P_Y0 * DTIME))**(ONE/P_M)
          
C          ELSE

            G_i = fac7 + ((TWO * P_H * P_R) / THREE) * xi_i**(P_R - ONE) 
     1          + (P_ETA0 / (P_M * DTIME)) * ((P_ETA0 * GAM_i) 
     2          / (P_Y0 * DTIME))**((ONE - P_M)/P_M)

            res_i = mod_beta_vp_tr - fac7 * GAM_i 
     1          - (P_Y0 + h * xi_i**P_R) * (TWO/THREE)**0.5D0 
     2          - P_Y0 * ((GAM_i * P_ETA0) / (P_Y0 * DTIME))**(ONE/P_M)
