...
C      List of internal variables:
       ...
       REAL(prec) :: treps, mod_beta_vp_tr, CHI_TR, GAM_i
       REAL(prec) :: fac1, zeta_trial, xi
       REAL(prec) :: fac2, res_i, G_i
       REAL(prec) :: ka, mu0, h, hh, y0, eta0, m
...
C     Calculate Beta_VP at trial
      BETA_VP_TR(:) = SIGMA_BAR_TR(:) - hh * EPS_VP(:)
      write(*,*) " "
      write(*,*) "beta_vp_trial"
      write(*,*) BETA_VP_TR(:)
      
      
C     Calculate euler norm of Beta_VP at trial
      CALL NORM(BETA_VP_TR, mod_beta_vp_tr)
      write(*,*) "||Beta_VP_tr|| : ", mod_beta_vp_tr
      
C     Calculate zeta at trial
      zeta_trial = hh * xi
      write(*,*) "zeta_trial : ", zeta_trial
...
C     Calculate Yield surface at trial
     ...
     1        - (y0 + zeta_trial) * (TWO/THREE)**0.5D0
...
C      Set residual to trial value of elastic domain boundary
       res_i = (CHI_TR - (TWO * mu0 + hh 
     1        + (TWO/THREE) * h) * GAM_i)**m 
     2         - (GAM_i * eta0) / DTIME

       G_i = (TWO * mu0 + hh + (TWO/THREE) * h) * m 
     1      * (CHI_TR - (TWO * mu0 + hh 
     2      + (TWO/THREE) * h) * GAM_i)**(m - ONE) + eta0/DTIME
...
C      Compute algorithmic Constants
       fac2 = (TWO * mu0 * GAM_i) / mod_beta_vp_tr
       fac1 = ((TWO * mu0) / G_i) - fac2
       
       DDSDDE(:, :) = TANG_TR(:, :) - (TWO * mu0) 
     1                      * (fac1 * retoret(:, :) + fac2 *xpp(:, :))
