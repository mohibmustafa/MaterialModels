C     UMAT - Isotropic Linear Viscoelastic - Perznya type J2 Viscoplastic with
C     Linear Isotropic, Kinematic Hardening with Lamitre type Ductile Damage   
...
C     Mohib Mustafa - IMDEA 4 MAR 2021      
     ...
     4     DFGRD1(3,3)
      ...
      call VE_VP_D(STRESS,STATEV,DDSDDE,STRAN,NTENS,NSTATV,PROPS,NPROPS,
...
C     !                   WITH LAMITRE TYPE DUCTILE DAMGE
C     !-------------------------------------------------------------------------
      SUBROUTINE VE_VP_D(STRESS,STATEV,DDSDDE,STRAN,NTENS,NSTATV,PROPS,
     &        NPROPS,DTIME,DSTRAN,KINC,KSTEP,NOEL, TIME)
      ...
      !  END DO
        ...
        ! write(*,*) " "
        ! write(*,*) "fac_ve1 : ", facve(1)
        ! write(*,*) "fac_ve2 : ", facve(2)
        ! write(*,*) "fac_ve3 : ", facve(3)
        ! write(*,*) "fac1 : ", fac1
        ! write(*,*) "fac2 : ", fac2
        ! write(*,*) "fac3 : ", fac3
        ! write(*,*) "fac4 : ", fac4
     ...
     2                  + BETA_VE_TR(:, 3)
        ...
        ! write(*,*) " "
        ! write(*,*) "beta_ve_trial_1"
        ! write(*,*) BETA_VE_TR(:, 1)

        ! write(*,*) " "
        ! write(*,*) "beta_ve_trial_2"
        ! write(*,*) BETA_VE_TR(:, 2)

        ! write(*,*) " "
        ! write(*,*) "beta_ve_trial_3"
        ! write(*,*) BETA_VE_TR(:, 3)

        ! write(*,*) " "
        ! write(*,*) "Sigma_dev_trial"
        ! write(*,*) SIGMA_DEV_TR(:)

        ! write(*,*) " "
        ! write(*,*) "beta_sc_trial : ", beta_sc_tr
     ...
     3                    + (DTIME/eta(3)) * BETA_VE_TR(:, 3)
          ...
          ! write(*,*) " "
          ! write(*,*) " Updated internal variables at trial state"
          ! write(*,*) " "
          ! write(*, *) "alpha1"
          ! write(*,*) STATEV(1 : 6)
          ! write(*, *) " "
     ...
     1                 * (ONE - D)
          ...
          ! write(*,*) " "
          ! write(*,*) " Updated return at trial state"
          ! write(*,*) " "
          ! write(*, *) "Stress"
          ! write(*,*) STRESS(:)
          ! write(*,*) " "
          ! write(*, *) "Consistent Tangent"
          ! DO K1 = 1, 6
          !   write(*,*) DDSDDE(K1, :)
          ! END DO
        ...
        ELSE
          ...
          !----------------------------------------------------------------
          !                NEWTON RAPHSON SOLVER FOR GAM_i
          !----------------------------------------------------------------
     ...
     1                     + (DTIME/eta(K1)) * BETA_VE_TR(:, K1)
          ...
          Y_e = mu(1) * dotProduct(E_e(:,1)) + mu(2) * dotProduct(E_e(:,2))
     1        + mu(3) * dotProduct(E_e(:,3))
     ...
     1          + 0.5d0 * h * xi_i ** TWO + Y_vphh
            ...
            ! write(*,*) " "
            ! Write(*,*) "------- Newton Raphson at : ",rap_iter, "--------"
            ! write(*,*) "For Gamma_i  = ", GAM_i
            ! write(*,*) " "
            ! write(*,*) "Internal Variables"
            ! write(*,*) " "
          ...
          write(*,*) " "
          Write(*,*) "------- Newton Raphson Converged ------------"
          ! write(*,*) "For Gamma_i  = ", GAM_i
          ! write(*,*) " "
          ! write(*,*) "Internal Variables"
          ! write(*,*) " "
          ...
          ! write(*,*) "BB5", BB5
          ! write(*,*) "BB6", BB6
          write(*,*) "Total iterations : ", rap_iter
          write(*,*) "Residual : ", res_i
          write(*,*) "G : ", G_i
          Write(*,*) "------------------------------------------------"
          ...
          ! WRITE(*,*) " "
          ! Write(*,*) "SIGMA (for matrix material)"
          ! WRITE(*,*) SIGMA(:)
          ! WRITE(*,*) " "
          ! Write(*,*) "STRESS (for abaqus)"
          ! WRITE(*,*) STRESS(:)
