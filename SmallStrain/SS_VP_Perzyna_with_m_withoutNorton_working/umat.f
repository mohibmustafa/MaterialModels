...
C      Set residual to trial value of elastic domain boundary
        res_i = (CHI_NP_TR - (TWO * P_MU + P_HH 
     1        + (TWO/THREE) * P_h) * GAM_i)**P_m 
     2         - (GAM_i * P_eta) / DTIME

        G_i = (TWO * P_MU + P_HH + (TWO/THREE) * P_h) * P_m 
     1      * (CHI_NP_TR - (TWO * P_MU + P_HH 
     2      + (TWO/THREE) * P_h) * GAM_i)**(P_m - ONE) + P_eta/DTIME    
        
        Write(*,*) "Gam_i before first iter", GAM_i
        write(*,*) "Res before 1st iter : ", res_i
        write(*,*) "G before 1st iter : ", G_i
     ...
     2         - (GAM_i * P_eta) / DTIME
          ...
          write(*,*) "Gam_i at ", rap_iter, " : ", GAM_i
          write(*,*) "Res at ", rap_iter, " : ", res_i
          write(*,*) "G at ", rap_iter, " : ", G_i
...
C       Calculate intermediate values
        ...
        BB2 = (TWO * P_MU * P_m / G_i) * (CHI_NP_TR - (TWO * P_MU + P_HH 
     2      + (TWO/THREE) * P_h) * GAM_i)**(P_m - ONE) 
