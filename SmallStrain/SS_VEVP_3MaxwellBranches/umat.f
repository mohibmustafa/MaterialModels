C Mohib Mustafa - IMDEA 18 Feb 2021      
C UMAT - Linear Isotropic Viscoelastic Viscoplastic - 3 Maxwell Branches
...
C     List of internal variables:
      ...
      DIMENSION ret(6), dev(6), BETA_TR(6, 3), BETA(6, 3)
      DIMENSION BETA_VP_TR(6), eps_ve_trial(6), BETA_TR_SUM(6)
      DIMENSION BETA_VP_PAR_TR(6), eps_vp_i(6), alpha_i(3, 6)
      DIMENSION STRAIN(6), SIGMA_TR(6), TANG_TR(6, 6), E1(6), E2(6)
      DOUBLE PRECISION treps, mod_beta_vp_tr, CHI_TR, gam, d_arc
      DOUBLE PRECISION fac2, fac4, mod_n, zeta_trial, y_i, sq_eps_e
      DIMENSION fac1(3), ETA(3), MU(3)
      DOUBLE PRECISION fac3, fac5, res_i, xi_i, ee1, ee2, evp, G_i, fac6
...
C     Difference of dev strain tensor at t n+1 and vp strain at n  
      DO K1 = 1, 6
        eps_ve_trial(K1)= dev(K1) - STATEV(18 + K1)
      END DO

C     Algorithmic factors
      DO K1 = 1, 3
        fac1(K1) = (TWO * MU(K1)) / (ONE + (TWO * MU(K1) * DTIME) / ETA(K1))
      END DO
      
      fac3 = TWO * P_MU0 + fac1(1) + fac1(2) + fac1(3)
      fac2 = fac3 + P_HH
...
C     Calculate Beta VP trial
      DO K1 = 1, 6
        BETA_VP_TR(K1) = BETA_VP_PAR_TR(K1) - P_HH * STATEV(K1 + 18)
      END DO      
...
C       Update Internal Variables
     ...
     1                          + (DTIME/ETA(K2)) * BETA(K1, K2)             
        ...
        DO K1 = 1, 6
          STATEV(K1 + 18) = STATEV(K1 + 18) + gam * ret(K1)
        END DO

        STATEV(25) =STATEV(25) + gam * (TWO / THREE)**0.5D0
...
C       Correct tangent modulus
        ...
        fac7 = gam * fac4
        fac6 = ((fac3) / (fac3 + P_HH + (TWO/THREE) * P_H 
     1       + P_ETA0/DTIME)) - fac7
       
        DO K1 = 1, 6
          DO K2 = 1, 6
            DDSDDE(K1, K2) = TANG_TR(K1, K2) - fac3 
     1                      * (fac6 * retoret(K1, K2) + fac7 *xpp(K1, K2))
          END DO
        END DO
      END IF
